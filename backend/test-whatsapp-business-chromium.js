...
console.log("TESTE WHATSAPP BUSINESS - WhatsApp Web.js (CHROMIUM)");
console.log("=".repeat(60));
console.log("Este teste usa Chromium do Puppeteer (não Chrome)");
console.log("Específico para contas WhatsApp Business");
console.log("=".repeat(60));

// Função para fechar processos do Chrome/Chromium no Windows
function closeChromeProcesses() {
  return new Promise((resolve) => {
    ...
    // Verificar se há processos do Chrome ou Chromium rodando
    exec('tasklist /FI "IMAGENAME eq chrome.exe" 2>nul', (error, stdout) => {
      const hasChrome = !error && stdout && stdout.includes('chrome.exe');
      
      exec('tasklist /FI "IMAGENAME eq chromium.exe" 2>nul', (error2, stdout2) => {
        const hasChromium = !error2 && stdout2 && stdout2.includes('chromium.exe');
        
        if (hasChrome || hasChromium) {
          console.log('⚠️  Processos do Chrome/Chromium encontrados. Fechando...');
          ...
          // Fechar processos do Chromium
          if (hasChromium) {
            exec('taskkill /F /IM chromium.exe 2>nul', (killError) => {
              if (!killError) {
                console.log('✅ Processos do Chromium fechados');
              }
            });
          }
...
async function cleanupSession(testSessionPath) {
  if (fs.existsSync(testSessionPath)) {
    ...
    // Fechar processos do Chrome/Chromium primeiro
    ...
    while (retries > 0) {
      try {
        ...
        if (error.code === 'EBUSY' && retries > 0) {
          ...
          console.warn('   Tente deletar manualmente a pasta: test_business_chromium_session');
          console.warn('   Ou feche todos os processos do Chrome/Chromium e tente novamente');
          break;
        }
      }
    }
...
async function main() {
  ...
  console.log('   (O Chromium será baixado automaticamente se necessário)');
  console.log('');

  // Limpar sessão anterior
  const testSessionPath = path.join(__dirname, 'test_business_chromium_session');
  ...
  // Configuração do Puppeteer FORÇANDO uso do Chromium
  // NÃO definir executablePath - deixar Puppeteer usar seu Chromium
  const puppeteerConfig = {
    ...
    // NÃO definir executablePath - usar Chromium do Puppeteer
    // executablePath: undefined, // Forçar uso do Chromium
  };

  console.log(`📦 Configuração: Chromium do Puppeteer (sem Chrome)`);
  console.log(`   Pasta de sessão: ${testSessionPath}`);

  // Criar cliente com estratégia de autenticação local
  // A pasta 'test_business_chromium_session' será criada para guardar os dados da sessão
  const client = new Client({
    authStrategy: new LocalAuth({
      dataPath: testSessionPath
    }),
    puppeteer: puppeteerConfig,
    // Não usar takeoverOnConflict - pode causar problemas
  ...
  client.on('ready', () => {
    ...
    const mensagem = 'Olá! Esta é uma mensagem de teste automatizada do seu bot com a conta Business usando Chromium. 🤖';
  ...
  client.on('auth_failure', msg => {
    ...
    console.error('\n💡 SOLUÇÃO:');
    console.error('   - Delete a pasta test_business_chromium_session e tente novamente');
  ...
  client.on('disconnected', (reason) => {
    ...
    if (reason === 'LOGOUT') {
      ...
      console.log('   - Deletar a pasta test_business_chromium_session e tentar novamente');
  ...
  // Inicia o cliente
  console.log("\n🚀 Inicializando cliente WhatsApp Business com Chromium...\n");
  console.log("⏳ Aguarde... O Chromium pode ser baixado na primeira execução.");
  console.log("");
  ...
  try {
    ...
    if (err.message.includes('Failed to launch') || err.message.includes('browser process')) {
      console.error('\n💡 SOLUÇÃO:');
      console.error('   - Execute: npm install puppeteer --save');
      console.error('   - O Chromium será baixado automaticamente pelo Puppeteer');
      console.error('   - Verifique sua conexão com a internet (para download do Chromium)');
      console.error('   - Feche todos os processos do Chrome/Chromium e tente novamente');
    } else if (err.message.includes('Chromium') || err.message.includes('browser')) {
      console.error('\n💡 SOLUÇÃO:');
      console.error('   - O Chromium pode não ter sido baixado corretamente');
      console.error('   - Execute: npm install puppeteer --save');
      console.error('   - Ou delete node_modules/.cache/puppeteer e tente novamente');
    }
