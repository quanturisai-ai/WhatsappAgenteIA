...
import { startScheduler } from './utils/vmLavScheduler';
...
// Whisper Service Manager (inicializado no startup se AUDIO_TRANSCRIPTION_ENABLED=true)
let whisperServiceManager: import('./utils/whisperServiceManager').WhisperServiceManager | null = null;
...
// Tratar requisições OPTIONS (preflight) com a mesma config
...
initDatabase()
  .then(async () => {
    ...
    // Verificar/iniciar Whisper Transcription Service (até 10 tentativas)
    if (process.env.AUDIO_TRANSCRIPTION_ENABLED === 'true') {
      try {
        const { WhisperServiceManager } = await import('./utils/whisperServiceManager');
        whisperServiceManager = new WhisperServiceManager();
        await whisperServiceManager.ensureRunning();
      } catch (error: any) {
        logger.warn(`⚠️  Whisper Service não disponível: ${error.message}`);
        logger.warn('   Áudios serão salvos sem transcrição');
      }
    }
    ...
    // Iniciar servidor HTTP PRIMEIRO para garantir que a API fique disponível
    httpServer.listen(PORT, () => {
      ...
      // Reconexão WhatsApp em background APÓS servidor estar ouvindo
      (async () => {
        try {
          ...
          for (const session of allSessions) {
            try {
              ...
              if (!fs.existsSync(userSessionPath)) continue;
              ...
              if (hasValidFiles) {
                logger.info(`✅ User ${session.user_id}: ${fileCount} arquivos de sessão. Reconectando em background...`);
                await new Promise(resolve => setTimeout(resolve, 3000));
                await sessionModel.updateByUserId(session.user_id, { status: 'connecting' });
                await whatsappManager.initializeService(session.user_id);
                reconnectedCount++;
                await new Promise(resolve => setTimeout(resolve, 2000));
              } else if (session.status === 'connected') {
                await sessionModel.updateByUserId(session.user_id, { status: 'connection_failed', qr_code: undefined });
              }
            } catch (error: any) {
          ...
          logger.warn(`Erro na reconexão automática WhatsApp: ${error.message}`);
        }
      })();
    });
...
const shutdown = async () => {
  ...
  // Encerrar Whisper Service se foi iniciado pelo backend
  if (whisperServiceManager) {
    try {
      whisperServiceManager.stop();
    } catch (error: any) {
      logger.error(`Erro ao encerrar Whisper Service: ${error.message}`);
    }
  }
