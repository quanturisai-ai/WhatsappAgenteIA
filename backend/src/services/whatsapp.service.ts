...
// que pode estar em um diretório inacessível (ex: C:\WINDOWS\system32\config\systemprofile\.cache\puppeteer)
if (!process.env.PUPPETEER_CACHE_DIR) {
  // Tentar usar o cache padrão do Puppeteer primeiro (onde o Chrome foi instalado)
  const defaultCacheDir = path.join(os.homedir(), '.cache', 'puppeteer');
  const fallbackCacheDir = path.join(os.homedir(), '.puppeteer_cache');
  
  // Usar o diretório que existe, ou criar o fallback
  const userCacheDir = fs.existsSync(defaultCacheDir) ? defaultCacheDir : fallbackCacheDir;
...
export class WhatsAppService extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // Detectar Google Chrome para melhor estabilidade (baseado no servermodelo.js)
      let chromeExec: string | undefined = undefined;
      if (process.platform === 'win32') {
        const CHROME_CANDIDATES = [
          // Caminhos diretos do executável
          'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',
          'C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe',
          // Diretórios base (anexaremos chrome.exe)
        ...
        ].filter(p => p); // Remover strings vazias
        
        for (const p of CHROME_CANDIDATES) {
          if (!fs.existsSync(p)) {
            logger.debug(`Chrome não encontrado em: ${p}`);
            continue;
          }
          try {
            ...
            if (stat.isDirectory()) {
              ...
              if (fs.existsSync(exe)) {
                chromeExec = exe;
                logger.info(`✅ Google Chrome encontrado em: ${chromeExec} para usuário ${this.userId}`);
                break;
              }
            ...
            logger.debug(`Erro ao verificar caminho ${p}: ${error.message}`);
          }
        }
        
        // 2. Se não encontrou no sistema, tentar encontrar Chrome instalado via Puppeteer
        if (!chromeExec) {
          logger.info(`Chrome não encontrado no sistema. Procurando Chrome instalado via Puppeteer...`);
          
          // Função recursiva para procurar chrome.exe em subdiretórios
          const findChromeInDir = (dir: string, depth: number = 0): string | null => {
            if (depth > 5) return null; // Limitar profundidade para evitar loops infinitos
          ...
          // Tentar múltiplos caminhos possíveis do cache do Puppeteer
          const puppeteerCachePaths = [
            path.join(os.homedir(), '.cache', 'puppeteer'), // Caminho padrão do Puppeteer (onde foi instalado)
            process.env.PUPPETEER_CACHE_DIR || path.join(os.homedir(), '.puppeteer_cache'),
          ...
          for (const cachePath of puppeteerCachePaths) {
            try {
              ...
              if (foundChrome && fs.existsSync(foundChrome)) {
                chromeExec = foundChrome;
                logger.info(`✅ Chrome instalado via Puppeteer encontrado em: ${chromeExec} para usuário ${this.userId}`);
                break;
              }
            } catch (error: any) {
              logger.debug(`Erro ao procurar Chrome no cache path ${cachePath}: ${error.message}`);
            }
          }
        }
        
        if (!chromeExec) {
          logger.warn(`⚠️ Google Chrome não encontrado no sistema nem no cache do Puppeteer.`);
          logger.warn(`💡 Tentando usar Chromium padrão do Puppeteer (será baixado automaticamente se necessário).`);
          logger.warn(`💡 Se o erro persistir, instale o Google Chrome ou execute: npx puppeteer browsers install chrome`);
        }
      }
      ...
      const puppeteerArgs = [
        ...
        '--disable-setuid-sandbox', // Importante para Windows
        '--disable-gpu', // Evita problemas de GPU
        '--no-first-run', // Evita inicializações desnecessárias
        '--no-zygote', // Evita processo zygote (pode causar problemas no Windows)
        '--disable-extensions', // Desabilita extensões que podem causar conflitos
        '--disable-default-apps', // Desabilita apps padrão
        '--disable-sync', // Desabilita sincronização do Chrome
        ...
        // IMPORTANTE: Flags adicionais para evitar erro de criptografia do Windows
        '--disable-password-manager', // Desabilita gerenciador de senhas
        '--disable-features=PasswordManager', // Desabilita recursos de senha
        // TEMPORARIAMENTE REMOVIDO: '--user-data-dir=' + path.join(userSessionPath, 'chrome_profile'), // Perfil isolado por usuário (evita conflitos de cache)
        // IMPORTANTE: Essas flags garantem isolamento do perfil e evitam conflitos de lock file
        // NOTA: Removido temporariamente para testar se está causando conflito com o cache do Puppeteer
      ];

      // Configurar Puppeteer - configuração mínima igual ao servermodelo.js
      const puppeteerConfig: any = {
        headless: true,
        args: puppeteerArgs,
        // Não definir timeout - deixar o Puppeteer usar o padrão
        // timeout: 60000, // Removido para evitar conflitos
   ...
   */
  private async getContactInfo(msg: Message): Promise<{ contactNumber: string; contactName: string }> {
    const msgData = (msg as any)._data || {};
    ...
    let contactNumber = rawIdentifier
      .replace('@c.us', '')
      .replace('@s.whatsapp.net', '')
      .replace('@g.us', '');

    // 2. Tentar obter nome do notifyName (mais confiável para nome de perfil)
    let contactName = msgData.notifyName || contactNumber;

    // 3. SE for um identificador @lid, tentar recuperar o número real
    if (rawIdentifier.includes('@lid')) {
      const lidClean = contactNumber.replace('@lid', '');
      logger.debug(`[LID Detection] Identificador @lid detectado: ${lidClean}. Tentando recuperar número real...`);
      ...
      try {
        ...
        if (chat && chat.name) {
          ...
          if (digitsInName.length >= 10) {
            ...
            // Manter o LID (sem o sufixo) como identificador único se não achou número
            contactNumber = lidClean;
        ...
        contactNumber = lidClean; // Fallback para o ID do LID sem o sufixo
      }
    }

    // Garantir que contactNumber não tenha o sufixo @lid no final se nada funcionou
    contactNumber = contactNumber.replace('@lid', '');
   ...
   */
  private detectMessageType(msg: Message): 'text' | 'media' | 'reaction' | 'system' | 'location' | 'contact' | 'other' {
    try {
  ...
  private async handleIncomingMessage(msg: Message): Promise<void> {
    try {
      ...
      const { contactNumber, contactName } = await this.getContactInfo(msg);
      ...
      // Preparar conteúdo para log e salvamento
      ...
      let conversation = await this.conversationModel.findByUserAndContact(
        this.userId,
        contactNumber
      );
      ...
      // Ignorar mensagens sem conteúdo que não sejam mídia ou localização
      if (!messageBody && messageType !== 'media' && messageType !== 'location' && messageType !== 'contact') {
      ...
      try {
        ...
        logger.info(`Mensagem salva no banco: ID=${savedMessage.id}, conversation_id=${conversation.id}, tipo=${messageType}, content="${bodyPreview}"`);
      } catch (error: any) {
      ...
      logger.info(`Mensagem recebida de ${contactName} (${contactNumber}): "${msg.body.substring(0, 50)}..."`);

      // Processar mensagem e responder automaticamente em background
      // NÃO USAR AWAIT aqui para não bloquear o processamento de mensagens
      // Isso evita que operações pesadas (RAG, Ollama) causem desconexão
      setImmediate(() => {
  ...
  async sendMessage(contactNumber: string, content: string, isFromAI: boolean = false): Promise<Message> {
    if (!this.client || !this.isReady()) {
      logger.error(`Tentativa de enviar mensagem falhou: cliente não está pronto. isInitialized=${this.isInitialized}, isClientReady=${this.isClientReady}, client=${this.client ? 'existe' : 'null'}`);
  ...
  async sendMedia(contactNumber: string, filePath: string, caption?: string, isFromAI: boolean = false, mediaId?: number, retryCount: number = 0): Promise<Message> {
    ...
    try {
      ...
      const message = await this.client.sendMessage(chatId, media, { caption: caption || '' });
  ...
  async disconnect(): Promise<void> {
    ...
    if (this.client) {
      ...
      logger.info(`WhatsApp desconectado para usuário ${this.userId}`);
    }
  }
  ...
  async logout(): Promise<void> {
    ...
    if (this.client) {
      try {
        // Tentar fazer logout - pode falhar no Windows se arquivos estiverem em uso
      ...
      // ✅ DELETAR ARQUIVOS DE SESSÃO (apenas aqui, no logout manual)
      try {
        const sessionPath = process.env.WHATSAPP_SESSION_PATH || './whatsapp_sessions';
        const userSessionPath = path.join(sessionPath, `user_${this.userId}`);
      ...
      logger.info(`WhatsApp logout realizado para usuário ${this.userId}`);
    }
  }
}
